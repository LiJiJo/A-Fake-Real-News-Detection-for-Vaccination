# Original Dataset. No Topic modelling
# -*- coding: utf-8 -*-
...
cdf.loc[13568, 'content'] = "Thirty seven people died in a day in New York City (3/10/20)"
cdf['Mixedcontent'] = cdf['title'].fillna('') + cdf['content']

X_test_vac_fulltxt = np.load('./Test_data of topic sorted dataset/X_testV.npy',allow_pickle=True)
# Converting the subset array into a Series with a compatible index
cdf = cdf[~cdf['Mixedcontent'].isin(X_test_vac_fulltxt)]
encoded_source=pd.get_dummies(cdf['source'])
content_column=cdf['Mixedcontent']
...
model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
history = model.fit(X_train, y_train, epochs=10, batch_size=32, validation_data=(X_test, y_test), class_weight={0: 1, 1: 2})


from tensorflow.keras.models import load_model
model.save('rnn_model_forOG')
model = load_model('rnn_model_forOG')
...
joblib.dump(nn_model, 'nn_modelOG.pkl')
joblib.dump(rf_model, 'rf_modelOG.pkl')
joblib.dump(xgb_model, 'xgb_modelOG.pkl')
joblib.dump(lr_model, 'lr_modelOG.pkl')

# Load each individual model
nn_model = joblib.load('nn_modelOG.pkl')
rf_model = joblib.load('rf_modelOG.pkl')
xgb_model = joblib.load('xgb_modelOG.pkl')
lr_model = joblib.load('lr_modelOG.pkl')
...
ensemble_predictions = ensemble2.predict(X_test)
...
############################################################################
#####################################################################
# Validate with Vaccine Topic Testing data
X_test_vac = np.load('./Test_data of topic sorted dataset/X_test.npy')
y_test_vac = np.load('./Test_data of topic sorted dataset/y_test.npy')

y_pred = ensemble.predict(X_test_vac)
confusion = confusion_matrix(y_test_vac, y_pred)
print("3NN Confusion Matrix:")
print(confusion)

rnn_predictions = model.predict(X_test_vac)
rnn_predictions = (rnn_predictions > 0.5).astype(int) 
confusion = confusion_matrix(y_test_vac, rnn_predictions)
print("RNN Confusion Matrix:")
print(confusion)

nn_predictions = nn_model.predict(X_test_vac)
nn_accuracy = accuracy_score(y_test_vac, nn_predictions)
print("Neural Network Accuracy:", nn_accuracy)
confusion = confusion_matrix(y_test_vac, nn_predictions)
print("NN Confusion Matrix:")
print(confusion)

lr_predictions = lr_model.predict(X_test_vac)
lr_accuracy = accuracy_score(y_test_vac, lr_predictions)
print("Logistic Regression Accuracy:", lr_accuracy)
confusion = confusion_matrix(y_test_vac, lr_predictions)
print("LR2 Confusion Matrix:")
print(confusion)

rf_predictions = rf_model.predict(X_test_vac)
rf_accuracy = accuracy_score(y_test_vac, rf_predictions)
print("Random Forest Accuracy:", rf_accuracy)
confusion = confusion_matrix(y_test_vac, rf_predictions)
print("RF Confusion Matrix:")
print(confusion)

xgb_predictions = xgb_model.predict(X_test_vac)
xgb_accuracy = accuracy_score(y_test_vac, xgb_predictions)
print("XGBoost Accuracy:", xgb_accuracy)
confusion = confusion_matrix(y_test_vac, xgb_predictions)
print("XGB Confusion Matrix:")
print(confusion)

ensemble_predictions = ensemble2.predict(X_test_vac)
ensemble_accuracy = accuracy_score(y_test_vac, ensemble_predictions)
print("Ensemble Accuracy:", ensemble_accuracy)
confusion = confusion_matrix(y_test_vac, ensemble_predictions)
print("Ensemble2 Confusion Matrix:")
print(confusion)
